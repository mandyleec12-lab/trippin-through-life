...
const FOCUSED_LANE_CURVES: Record<number, LanePoint[]> = {
  // Bottom -> top in normalized world coordinates. Curves stay mostly forward
  // so movement feels integrated with the cinematic road perspective.
  0: [{
    x: 43,
    y: 93
  }, {
    ...
    perspectiveOrigin: '50% 78%'
      ...
      ease: [0.22, 1, 0.36, 1]
        ...
        return <Fragment key={laneIdx}>
            ...
            return <motion.div key={`${laneIdx}-${tileId}-${idx}`} className="absolute border rounded-md flex items-center justify-center overflow-hidden" style={{
              left: point.x,
              top: point.y,
              width,
              height,
              ...
              boxShadow: isCurrent ? `0 0 18px ${neon}66, inset 0 1px 0 rgba(255,255,255,0.22), inset 0 -8px 12px rgba(0,0,0,0.42)` : `0 2px 8px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.12)`,
              ...
              duration: 0.45
                    ...
                    backgroundImage: 'repeating-linear-gradient(90deg, rgba(255,255,255,0.1) 0 2px, transparent 2px 6px)'
                  }} />
                      <div className="absolute inset-x-[8%] top-1/2 h-px -translate-y-1/2 bg-white/30" />
                      <div className="absolute left-[12%] top-1/2 h-[55%] w-[2px] -translate-y-1/2 rounded-full" style={{
                    background: laneColor,
                    boxShadow: `0 0 10px ${laneColor}`,
                    opacity: isCurrent ? 0.95 : 0.65
                  }} />
                      {showText ? <div className="relative z-[1] flex max-w-full items-center gap-1.5 px-2 text-white/90">
                        ...
                        </div> : <div className="relative z-[1] h-1.5 w-1.5 rounded-full bg-white/80" style={{
                    boxShadow: `0 0 7px ${laneColor}`
                  }} />}
                    </div>
                  </motion.div>;
          })}
