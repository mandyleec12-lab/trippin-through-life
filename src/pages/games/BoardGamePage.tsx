...
const getTileById = (id: number): TileData => {
  return TILES.find((t) => t.id === id) || TILES[0];
};
// Position calculator for the 3-path linear layout
const getBoardPosition = (tileId: number, activePathTiles: number[][] = DEFAULT_PATH_TILE_ASSIGNMENTS): {
  x: number;
  y: number;
  angle: number;
} => {
...
}: {value: number;style: React.CSSProperties;}) {
...
function getDiceFaceRotation(value: number | null): {
  x: number;
  y: number;
} {
  switch (value) {
...
export function BoardGamePage() {
  ...
  const [taxResults, setTaxResults] = useState<Array<{
    name: string;
    emoji: string;
    taxType: string;
    taxed: number;
  ...
  // Realm State
  const [realmEvents, setRealmEvents] = useState<Array<{
    name: string;
    amount: number;
  }>>([]);
  ...
  const generateShuffledPaths = () => {
    ...
    const PATH_SPECIFIC: Record<number, number[]> = {
      0: [80, 81, 82, 83, 84],
      1: [85, 86, 87, 88, 89],
      2: [90, 91, 92, 93, 94]
    };
    const PATH_ANCHOR_TILES: Record<number, number> = {
      0: 100,
      1: 102,
      2: 101
    };
  ...
  const dealStartingHands = () => {
    ...
    const chaosLimits: Record<number, number> = {
      0: 3,
      1: 2,
      2: 3
    };
  ...
  // --- JOB EFFECTS ---
  const applyJobPreRollEffect = (playerIndex: number): boolean => {
    const player = players[playerIndex];
  ...
  const handleCareerSwitchSelection = (pathIndex: number) => {
            ...
            delay: index * 0.05
                ...
                if (f) handleAvatarUpload(player.id, f);
                  ...
                  <select value={player.color} onChange={(e) => updatePlayer(player.id, 'color', e.target.value as PlayerColor)} className="bg-white/80 border border-purple-200/50 rounded-xl px-2 py-1.5 text-purple-700 font-bold outline-none text-sm backdrop-blur-sm">
                    ...
                    {(['pink', 'purple', 'blue', 'teal', 'gold', 'coral'] as PlayerColor[]).map((c) => <option key={c} value={c}>
