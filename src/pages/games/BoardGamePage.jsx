...
import { PLAYER_COLORS, PLAYER_BORDER, CATEGORY_STYLES, LIFE_PATHS, EDUCATION_PATH_COUNT, STARTING_MODIFIERS, PATH_JOBS, DEFAULT_PATH_TILE_ASSIGNMENTS, CAREER_SWITCH_EVENTS, CHAOS_REALM_SCENES } from '../../lib/gameData';
import { ALL_JOBS, TILES } from '../../lib/gameTiles';
import LifeCrossroadsModal, { CROSSROADS_QUESTIONS } from '../../components/games/LifeCrossroadsModal';
import WinnerScreen from '../../components/games/WinnerScreen';
import TaxEventScreen from '../../components/games/TaxEventScreen';
import CareerSwitchScreen from '../../components/games/CareerSwitchScreen';
import ChaosRealmScreen from '../../components/games/ChaosRealmScreen';

const PAWN_STEP_DURATION_MS = 440;
const PAWN_LANDING_BUFFER_MS = 700;
const STARTING_HAND_RESHUFFLE_LIMIT = 2;
// ALL_JOBS and TILES imported from lib/gameTiles
const _DUMMY_JOBS_REMOVED = null; // was: [
const _DUMMY_JOBS_PLACEHOLDER = {
  id: 'engineer',
  name: 'Engineer',
  emoji: '🛠️',
  wage: 45,
...
// Constants imported from gameData
...
const TRANSPORTATION_EVENTS = [{name:'Low on gas… fill up ⛽',amount:-60,emoji:'⛽'},{name:'Gas prices hit again 😒',amount:-80,emoji:'⛽'},{name:'Oil change due 🛠️',amount:-75,emoji:'🛠️'},{name:'Tire pressure issue… quick fix',amount:-40,emoji:'🔧'},{name:'Battery low… need to charge 🔌',amount:-30,emoji:'🔌'},{name:'Charging station cost more than expected 😬',amount:-50,emoji:'🔌'},{name:'Took an Uber… surge pricing 😭',amount:-45,emoji:'🚕'},{name:'Late night ride cost more than expected 🚕',amount:-60,emoji:'🚕'},{name:'Had to Uber instead of driving',amount:-35,emoji:'🚕'},{name:'Bus/train fare 🚌',amount:-10,emoji:'🚌'},{name:'Had to reload transit card',amount:-25,emoji:'🚌'},{name:'Missed ride… paid again 😒',amount:-15,emoji:'🚌'},{name:'Car insurance went up 😤',amount:-90,emoji:'🚗'},{name:'Parking garage fee 🅿️',amount:-35,emoji:'🅿️'},{name:'Car registration renewal 📋',amount:-120,emoji:'📋'},{name:'Windshield wiper replacement',amount:-30,emoji:'🚗'},{name:'Uber pool still expensive 😒',amount:-28,emoji:'🚕'},{name:'Train delay made you late',amount:-20,emoji:'🚌'},{name:'Toll road surprise 💸',amount:-15,emoji:'🛣️'},{name:'Had to jump start the car 🔋',amount:-50,emoji:'🔋'}];

// getJobsForPath uses PATH_JOBS imported from lib/gameData
...
export function BoardGamePage() {
  ...
  return <div className="min-h-screen bg-gradient-to-br from-pink-100 via-lavender-100 to-blue-100 overflow-hidden font-sans select-none">
      ...
      {/* ========== CHAOS REALM ========== */}
      {phase === 'chaos_realm' && <ChaosRealmScreen showChaosTransition={showChaosTransition} currentChaosScene={currentChaosScene} chaosSceneIndex={chaosSceneIndex} chaosSceneCount={chaosSceneCount} realmTotalLoss={realmTotalLoss} chaosOutcome={chaosOutcome} onChoice={handleChaosChoice} onContinue={continueChaosRealm} />}

      {/* ========== CAREER SWITCH ========== */}
      {phase === 'career_switch' && <CareerSwitchScreen realmEvents={realmEvents} lifePaths={LIFE_PATHS} pendingNewPath={pendingNewPath} onSelectPath={handleCareerSwitchSelection} onConfirm={confirmCareerSwitch} getJobsForPath={getJobsForPath} />}

      {/* ========== TAX EVENT ========== */}
      {phase === 'tax_event' && <TaxEventScreen taxResults={taxResults} onClose={closeTaxEvent} />}

      {/* ========== WINNER ========== */}
      {phase === 'winner' && <WinnerScreen winner={winner} players={players} onPlayAgain={() => {
        setPhase('setup');
        setIsPawnAnimating(false);
        setPlayers([{id:'1',name:'Player 1',color:'pink',position:0,skipNextTurn:false,avatar:null,job:null,money:0,turnsPlayed:0,roundsWithoutIncome:0,evolved:false,pathIndex:null,inSchool:false,schoolTurnsLeft:0,studentLoanDebt:0,chaosTriggersRemaining:0,startingModifier:null,crossroadsDecisions:[]},{id:'2',name:'Player 2',color:'purple',position:0,skipNextTurn:false,avatar:null,job:null,money:0,turnsPlayed:0,roundsWithoutIncome:0,evolved:false,pathIndex:null,inSchool:false,schoolTurnsLeft:0,studentLoanDebt:0,chaosTriggersRemaining:0,startingModifier:null,crossroadsDecisions:[]}]);
      }} />}

      {/* ========== LIFE CROSSROADS ========== */}
      <AnimatePresence>
        {activeCrossroads && <LifeCrossroadsModal player={players[currentPlayerIndex]} question={activeCrossroads} onChoice={handleCrossroadsChoice} />}
      </AnimatePresence>
    </div>;
}